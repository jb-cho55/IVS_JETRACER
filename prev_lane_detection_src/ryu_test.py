...
def process_frame(frame):
    ...
    # Determine target curvature (or target radius)
    radius_of_90_curve = 30  # Example radius in meters
    target_curvature = 1 / radius_of_90_curve

    # Calculate the curvature error
    curvature_error = curvature - target_curvature

    # Use PID controller to compute the steering value
    steering = pid_controller.update(curvature_error)

    # Apply logarithmic scaling and clamping
    if steering > 0:
        steering = np.log10(steering + 1)  # Add 1 to avoid log(0) and scale
    elif steering < 0:
        steering = -np.log10(-steering + 1)  # Add 1 to avoid log(0) and scale
    steering = np.clip(steering, -1, 1)  # Ensure steering is in range [-1, 1]

    # Inverting sign for left turn
    if left_curverad < right_curverad:
        steering = -steering

    # Display the steering value on the frame
    cv2.putText(frame, "Steering: {:.2f}".format(steering), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def main():
    ...
    ki = 0.0
    kd = 0.1
    dt = 1.0 / 30  # Assuming a frame rate of 30 FPS
    pid_controller = PIDController(kp, ki, kd, dt)
    
    # Open video file
    video_path = 'example_2.mp4'  # Update with your video file path
